...
static void WBufAppendUTF8Codepoint(WBuf* b, uint32_t codepoint) {
  if (codepoint <= 0x7f) {
...
static void writeAttribute(JsonFormatter* r, const MD_ATTRIBUTE* attr) {
  for (uint32_t i = 0; attr->substr_offsets[i] < attr->size; i++) {
    MD_TEXTTYPE type = attr->substr_types[i];
    MD_OFFSET   off  = attr->substr_offsets[i];
    MD_SIZE     size = attr->substr_offsets[i+1] - off;
    const MD_CHAR* text = attr->text + off;
...
int fmt_json(const MD_CHAR* input, MD_SIZE input_size, WBuf* outbuf, uint32_t parser_flags) {
