...
static inline size_t fmtu32(uint32_t n, uint32_t radix, char* buf) {
  if (n == 0) {
    *buf = '0';
    return 1;
  }
  ...
  if (len <= Z/2) {
    ...
    uint32_t* inp = (uint32_t*)p;
    uint32_t* out = (uint32_t*)buf;
    const uint32_t* end = (uint32_t*)(buf + Z);
    while (inp < end) {
      *out++ = *inp++;
    }
  } else if (len < Z) {
...
void WBufAppendU32(WBuf* b, uint32_t n, uint32_t radix) {
  WBufReserve(b, 32);
