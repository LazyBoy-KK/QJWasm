...
use runtime::{InstanceInner, SendSyncPtr, VMContext, VMFuncRef};
use wiggle::{GuestMemory, run_in_dummy_executor};

use crate::object_impl::store::StateData;

macro_rules! generate_wasi_func {
    (
        $(
            $( #[$docs:meta] )*
        ...
        /// Adds the Wasi functions to the given linker.
        $(
            pub unsafe extern "C-unwind" fn $fname(vmctx: *mut VMContext, $($arg : $typ,)*) -> $ret
            {
                InstanceInner::from_vmctx(vmctx, |inner| {
                    let result = async {
                        let state_data: &StateData = inner.host_state().unwrap();
                        let state = state_data.get_mut_state();
                        let ctx = state.get_wasi_ctx();
                        let mut memory = inner.get_memory().unwrap();
                        let mut guest = GuestMemory::Unshared(memory.data_mut());
                        match wasi_common::snapshots::preview_1::wasi_snapshot_preview1::$fname(ctx, &mut guest, $($arg,)*).await {
                            Ok(r) => <$ret>::from(r),
                            Err(e) => match e.downcast::<wasi_common::I32Exit>() {
                                Ok(wasi_common::I32Exit(status)) => runtime::panic_any(runtime::ProcExitCode(status)),
                                Err(e) => runtime::panic_any(e.to_string()),
                            }
                        }
                    };
                    run_in_dummy_executor(result).unwrap()
                })
            }
        )*
    }
...
macro_rules! make_wasi_map {
    ( $( $func:ident ),* $(,)? ) => {
        pub fn get_wasi_map() -> HashMap<&'static str, VMFuncRef> {
            let mut map = HashMap::new();
            $(
                let ptr = NonNull::new($func as *mut c_void).unwrap();
                let native_call = SendSyncPtr::new(ptr);
                let funcref = VMFuncRef {
                    native_call: native_call.as_ptr(),
                    vmctx: Default::default(),
                    type_index: Default::default(),
                    offset: Default::default(),
                    caller: Default::default(),
                };

                map.insert(stringify!($func), funcref);
            )*
            map
        }
    };
...
generate_wasi_func!(
    ...
    fn path_readlink(fd: i32, offset: i32, length: i32, buf: i32, buf_len: i32, offset0: i32) -> i32;
    ...
    fn path_rename(fd: i32, old_offset: i32, old_length: i32, new_fd: i32, new_offset: i32, new_length: i32) -> i32;
    ...
    fn path_symlink(old_offset: i32, old_length: i32, fd: i32, new_offset: i32, new_length: i32) -> i32;
    ...
    fn sock_recv(fd: i32, iov_buf: i32, iov_buf_len: i32, ri_flags: i32, offset0: i32, offset1: i32) -> i32;
